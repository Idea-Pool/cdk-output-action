...
import {ActionProps} from '../src/input'

function runAction(props?: ActionProps) {
  process.env.INPUT_OUTPUT_PATH =
    props?.outputPath || './__tests__/cdk-outputs.json'
  ...
  process.env.INPUT_APPEND_STACK_NAME =
    props?.appendStackName?.toString() || 'false'
  process.env.INPUT_TRIM_UUID = props?.trimUUID?.toString() || 'true'
...
test('test runs', () => {
  try {
    ...
    console.log(e)
    console.log((e as unknown as any).stdout.toString())
  }
})
